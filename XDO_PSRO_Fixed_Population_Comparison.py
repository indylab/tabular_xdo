...
def get_xdo_restricted_game_meta_Nash(game, br_list, br_conv_threshold=1e-2, seed=1):
    ...
    for j in range(int(1e10)):
        ...
        if j%50==0:
            ...
            np.save('./results/fixed_pop/XDO/num_pop_'+str(len(br_list))+'_seed_'+str(seed)+'_times.npy', np.array(cfr_psro_times))
            np.save('./results/fixed_pop/XDO/num_pop_'+str(len(br_list))+'_seed_'+str(seed)+'_exps.npy', np.array(cfr_psro_exps))
            np.save('./results/fixed_pop/XDO/num_pop_'+str(len(br_list))+'_seed_'+str(seed)+'_episodes.npy', np.array(cfr_psro_episodes))
            np.save('./results/fixed_pop/XDO/num_pop_'+str(len(br_list))+'_seed_'+str(seed)+'_infostates.npy', np.array(cfr_psro_infostates))            
